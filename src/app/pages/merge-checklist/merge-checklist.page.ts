...
export class MergeChecklistPage implements OnInit {
	...
	public todo = [
		{
			val: 'Run your own execution client',
			isChecked: false,
			key: 'cl_own_client',
			timing: 'Timing: Do this now',
		},
		{
			val: 'Update execution & consensus client',
			isChecked: false,
			key: 'cl_update',
			timing: 'Timing: Do this after TTD client releases',
		},
		{
			val: 'JWT authentication for Engine API',
			isChecked: false,
			key: 'cl_auth',
			timing: 'Timing: Do this after TTD client releases',
		},
		{
			val: 'Set fee recipient address',
			isChecked: false,
			key: 'cl_fee_recipient',
			timing: 'Timing: Do this after TTD client releases',
		},
		{
	...
	change(entry) {
		...
		this.storage.setBooleanSetting(
			entry.detail.value.key,
			entry.detail.value.isChecked
		)
	}
