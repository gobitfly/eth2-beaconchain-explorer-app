...
import {
	BlockUtils,
	ETHPOOL,
	ROCKETPOOL_SMOOTHING_POOL,
} from 'src/app/utils/BlockUtils'
...
export class BlockDetailPage implements OnInit {
	...
	async ngOnInit() {
		...
		this.producerReward = await this.blockUtils.getBlockRewardWithShare(
			this.block
		)
		...
		this.baseFee =
			new BigNumber(this.block.baseFee).dividedBy(1e9).toFixed(1) + ' Gwei'
		this.gasUsedPercent = (
			(this.block.gasUsed * 100) /
			this.block.gasLimit
		).toFixed(1)
		this.burned = new BigNumber(this.block.gasUsed).multipliedBy(
			new BigNumber(this.block.baseFee)
		)
	...
	private getBlockies() {
		...
		const dataurl = blockies
			.create({ seed: this.block.blockHash, size: 8, scale: 7 })
			.toDataURL()
		return dataurl
	}
	...
	async openBlock() {
		await Browser.open({
			url:
				(await this.getBaseBrowserUrl()) + '/block/' + this.block.blockNumber,
			toolbarColor: '#2f2e42',
		})
	}
