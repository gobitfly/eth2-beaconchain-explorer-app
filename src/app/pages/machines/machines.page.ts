...
export class MachinesPage extends MachineController implements OnInit {
	...
	openTimeSelection() {
		if (!this.hasHistoryPremium) {
			this.alertService.showInfo(
				'Premium Feature',
				'For more machine history upgrade to a premium version.'
			)
			return
		}
	...
	async getAndProcessData() {
		...
		this.data = await this.machineUtils.getAndProcessData(
			this.getTimeSelectionLimit()
		)
	...
	async checkForLegacyApi(data: ProcessedStats[]) {
		const dismissed = await this.storage.getBooleanSetting(
			'legacy_monitoring_api_dismissed',
			false
		)
		if (dismissed || !data) {
