...
import {
	DEBUG_SETTING_OVERRIDE_PACKAGE,
	StorageService,
...
export class DevPage extends Tab3Page implements OnInit {
	...
	ngOnInit() {
		...
		this.storage
			.getSetting(DEBUG_SETTING_OVERRIDE_PACKAGE, 'default')
			.then((result) => {
		...
		this.firebaseUtils
			...
			.then((result) => (this.notificationConsent = result))
	}
	...
	clearApiCache() {
		...
		this.alerts.confirmDialog(
			'Restart',
			'API requests cache cleared, restart?',
			'OK',
			() => {
				this.restartApp()
			}
		...
		setTimeout(
			() =>
				this.alerts.showInfo(
					'Success',
					'Toggle test was successfull if this alert only appears once and toggle returns to disabled'
				),
			650
		)
	}
	...
	clearStorage() {
		this.alerts.confirmDialog(
			'Clear Storage',
			'All app data will be removed, continue?',
			'OK',
			() => {
				this.storage.clear()
				Toast.show({
	...
	changePackage() {
		...
		this.alerts.confirmDialog(
			'Restart',
			'Requires restart to take affect, restart?',
			'OK',
			() => {
				this.restartApp()
			}
	...
	async restoreAuthUser() {
		...
		this.alerts.confirmDialog(
			'Success',
			'Restart app with restored user?',
			'OK',
			() => {
				this.restartApp()
			}
