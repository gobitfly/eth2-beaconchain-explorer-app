...
export class SubscribePage implements OnInit {
	...
	async continuePurchaseIntern() {
		...
		if (isPremium) {
			const packageName = capitalize(
				await this.merchant.getCurrentPlanConfirmed()
			)
			this.alertService.confirmDialog(
				'Already Premium',
				'You already own the ' +
					packageName +
					' package, are you sure that you want to continue with your purchase?',
				'Yes',
				async () => {
					if (isNoGoogle) {
						await Browser.open({
							url: 'https://beaconcha.in/premium',
							toolbarColor: '#2f2e42',
						})
					} else {
	...
	async purchaseIntern() {
		...
		if (!loggedIn) {
			this.alertService.confirmDialog(
				'Login',
				'You need to login to your beaconcha.in account first. Continue?',
				'Login',
				() => {
