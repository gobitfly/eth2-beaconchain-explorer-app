...
export class MachineDetailPage extends MachineController implements OnInit {
	...
	async ngOnInit() {
		...
		if (this.data) {
			...
			this.os = this.formatOS(
				this.getLastFrom(this.data.system, (array) => array.misc_os)
			)
			this.uptime =
				this.getLastFrom(
					this.data.system,
					(array) => array.misc_node_boot_ts_seconds
				) * 1000
			this.threadCount = this.getLastFrom(
				this.data.system,
				(array) => array.cpu_threads
			)
			this.coreCount = this.getLastFrom(
				this.data.system,
				(array) => array.cpu_cores
			)

			this.headslot = this.getLastFrom(
				this.data.node,
				(array) => array.sync_beacon_head_slot
			)

			let synced = this.getLastFrom(
				this.data.node,
				(array) => array.sync_eth2_synced
			)
			this.stateSynced = synced ? 'Synced' : 'Syncing'

			this.validatorLabelActive =
				'Active: ' +
				this.getLastFrom(this.data.validator, (array) => array.validator_active)
			this.validatorLabelTotal =
				'Total: ' +
				this.getLastFrom(this.data.validator, (array) => array.validator_total)

			const lastFreeBytes = this.getLastFrom(
				this.data.system,
				(array) => array.disk_node_bytes_free
			)
			const totalBytes = this.getLastFrom(
				this.data.system,
				(array) => array.disk_node_bytes_total
			)
			const percent = Math.round((lastFreeBytes * 1000) / totalBytes) / 10
			this.diskLabel =
				'Free: ' + bytes(lastFreeBytes, true, true, 1) + ' - ' + percent + '%'
			this.beaconchainLabel =
				'Size: ' +
				bytes(
					this.getLastFrom(
						this.data.node,
						(array) => array.disk_beaconchain_bytes_total
					),
					true,
					true,
					3
			...
			this.peerLabel =
				...
				this.getLastFrom(
					this.data.node,
					(array) => array.network_peers_connected
				)
			...
			this.networkLabelRx =
				...
				bytes(
					this.getAvgFrom(
						this.data.system,
						(array) =>
							array.network_node_bytes_total_receive / this.magicGapNumber,
						true
					),
					true,
					true,
			...
			this.networkLabelTx =
				...
				bytes(
					this.getAvgFrom(
						this.data.system,
						(array) =>
							array.network_node_bytes_total_transmit / this.magicGapNumber,
						true
					),
					true,
					true,
			...
			const lastMemTotal = this.getLastFrom(
				this.data.system,
				(array) => array.memory_node_bytes_total
			)
			const percentMem =
				Math.round(1000 - (lastMemAv * 1000) / lastMemTotal) / 10
			this.memoryLabelFree =
				'Used: ' +
				bytes(lastMemTotal - lastMemAv, true, true, 1) +
				' - ' +
				percentMem +
				'%'
			...
			this.memoryProcessLabelNode =
				...
				bytes(
					this.getLastFrom(
						this.data.node,
						(array) => array.memory_process_bytes
					),
					true,
					true,
					2
			...
			this.cpuProcessLabelNode =
				...
				(
					this.getAvgRelativeFrom(
						this.getLastN(
							this.data.node,
							(array) => array.cpu_process_seconds_total,
							true
						),
						this.getLastN(
							this.data.system,
							(array) => array.cpu_node_system_seconds_total,
							true
						),
						(val1, val2) => {
							return val1 / val2
			...
			this.cpuLabelTotal =
				...
				(
					...
					this.getAvgRelativeFrom(
						this.getLastN(
							this.data.system,
							(array) => array.cpu_node_idle_seconds_total,
							true
						),
						this.getLastN(
							this.data.system,
			...
			this.diskUsageLabelReads =
				...
				Math.round(
					this.getAvgFrom(
						this.data.system,
						(array) => array.disk_node_reads_total / this.magicGapNumber,
						true
					)
				) +
				' iops'
			this.diskUsageLabelWrites =
				...
				Math.round(
					this.getAvgFrom(
						this.data.system,
						(array) => array.disk_node_writes_total / this.magicGapNumber,
						true
					)
				) +
				' iops'
			...
			this.syncLabelEth1Connected = eth1Connected
				? 'ETH1 Connected'
				: 'ETH1 Offline'

			let fulylSynced = this.getLastFrom(
				this.data.node,
				(array) => array.sync_eth2_synced
			)
