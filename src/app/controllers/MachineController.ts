...
export default class MachineController {
			...
			config: {
				...
				yAxis: {
					labels: {
						...
						formatter: function () {
							return (
								bytes(this.value, true, this.isFirst, 0) + (perS ? '/s' : '')
							)
						},
					},
				},
	...
	public getFallbackConfigurations(
		data: ProcessedStats
        ...
        */
		...
		let eth2FallbackConfigured = this.getLastFrom(
			data.node,
			(array) => array.sync_eth2_fallback_configured
		)
		let eth1FallbackConfigured = this.getLastFrom(
			data.node,
			(array) => array.sync_eth1_fallback_configured
		)
		return {
	...
	isBuggyPrysmVersion(data: ProcessedStats): boolean {
		return (
			data.client == 'prysm' &&
			(!data.system || data.system.length <= 2 || data.system[0].cpu_cores == 0)
		)
	}
	...
	protected async getDiskAttention(data: ProcessedStats): Promise<string> {
		...
		let freePercentage = this.getLastFrom(
			data.system,
			(array) => array.disk_node_bytes_free / array.disk_node_bytes_total
		)
		...
		if (freePercentage < threshold / 100) {
			return (
				"Your disk is almost full. There's less than " +
				threshold +
	...
	protected async getMemoryAttention(data: ProcessedStats): Promise<string> {
		...
		let usagePercentage =
			...
			this.getLastFrom(
				data.system,
				(array) =>
					(array.memory_node_bytes_free +
						array.memory_node_bytes_buffers +
						array.memory_node_bytes_cached) /
					array.memory_node_bytes_total
			)
		...
		if (usagePercentage > threshold / 100) {
			return (
				'Your RAM usage exceeded ' + threshold + '% of the available space.'
			)
		}
	...
	protected getSyncAttention(data: ProcessedStats): string {
		...
		let eth1Connected = this.getLastFrom(
			data.node,
			(array) => array.sync_eth1_connected
		)

		let fallbacksConfigured = this.getFallbackConfigurations(data)
		let eth2Fallback = this.getLastFrom(
			data.node,
			(array) => array.sync_eth2_fallback_connected
		)
		let eth1Fallback = this.getLastFrom(
			data.node,
			(array) => array.sync_eth1_fallback_connected
		)
	...
	protected getLastN<T>(
		dataArray: T[],
		callbackValue: (array: T) => any,
		isDiffPair: boolean = false,
		depth = 10
	) {
		var erg = []
		...
		for (var i = 1; i < length; i++) {
			if (isDiffPair) {
				erg[i - 1] =
					callbackValue(dataArray[dataArray.length - i]) -
					callbackValue(dataArray[dataArray.length - (i + 1)])
			} else {
	...
	protected getAvgRelativeFrom(
		data1LastN: any[],
		data2LastN: any[],
		callback: (val1, val2) => any
	) {
		...
		console.log(
			'getAvgRelativeFrom',
			Math.round((erg / length) * 10000) / 10000
		)
		return Math.round((erg / length) * 10000) / 10000
	}

	protected getLastFrom<T>(
		dataArray: T[],
		callbackValue: (array: T) => any,
		isDiffPair: boolean = false
	): any {
		...
		if (isDiffPair) {
			...
			return (
				callbackValue(dataArray[dataArray.length - 1]) -
				callbackValue(dataArray[dataArray.length - 2])
			)
		} else {
	...
	getGapSize(dataSet: StatsBase[]) {
		...
		if (isNaN(temp) || temp < 30000)
			temp = Math.abs(dataSet[0][0] - dataSet[1][0])
		if (isNaN(temp) || temp < 30000)
			temp = Math.abs(
				dataSet[dataSet.length - 2][0] - dataSet[dataSet.length - 1][0]
			)
	...
	timeAxisRelative(
		...
		while (true) {
			...
			let tempValue =
				maxV[1] == 0
					? 0
					: Math.round((curV[1] / maxV[1]) * (100 * rounding)) / rounding
	...
	public timeAxisChanges<T extends StatsBase>(
		data: T[],
		delegateValue: (value: T, timeDiff?) => number,
		accumilative: boolean = false
	) {
		var result = []
		var summedUp = -1
		var lastValue = 0
		...
		data.forEach((value) => {
			...
			const temp = accumilative
				? Math.round((current - summedUp) * 100) / 100
				: current

			if (
				lastTimestamp != -1 &&
				lastTimestamp + this.selectionTimeFrame * 60 * 1000 < value.timestamp
			) {
				...
				if (
					lastTimestamp != -1 &&
					lastTimestamp + 45 * 60 * 1000 < value.timestamp
				) {
					console.log(
						'filling empty plots with zeros: ',
						lastTimestamp,
						value.timestamp
					)
	...
	public combineByMachineName(validator: any[], node: any[], system: any[]) {
		...
		for (var key in allKeys) {
			...
			const unixTime = this.findAnyData(
				sortedVal,
				sortedNode,
				sortedSystem,
				(value) => {
			...
			result[key] = {
				...
				clientVersion: this.findAnyData(
					sortedVal,
					sortedNode,
					sortedSystem,
					(value) => {
