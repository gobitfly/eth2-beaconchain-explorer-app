...
export default class OverviewController {
	...
	private process(
		...
		if (!validators || validators.length <= 0 || currentEpoch == null)
			return null

		const effectiveBalance = sumBigInt(
			validators,
			(cur) => cur.data.effectivebalance
		)
		const effectiveBalanceActive = sumBigInt(validators, (cur) => {
			return cur.data.activationepoch <= currentEpoch.epoch
				? cur.data.effectivebalance
				: new BigNumber(0)
		})
		...
		const aprPerformance31dConsensus = sumBigInt(
			validators,
			(cur) => cur.data.performance31d
		)
		const aprPerformance31dExecution = sumBigInt(validators, (cur) =>
			this.sumExcludeSmoothingPool(cur, (cur) =>
				cur.execution.performance31d.toString()
			)
		)

		const overallBalance = this.sumBigIntBalanceRpl(
			validators,
			(cur) => new BigNumber(cur.data.balance)
		)
		...
		const combinedPerf = {
			performance1d: consensusPerf.performance1d.plus(
				executionPerf.performance1d
			),
			performance31d: consensusPerf.performance31d.plus(
				executionPerf.performance31d
			),
			performance7d: consensusPerf.performance7d.plus(
				executionPerf.performance7d
			),
			performance365d: consensusPerf.performance365d.plus(
				executionPerf.performance365d
			),
			apr: this.getAPRFromMonth(
				effectiveBalanceActive,
				aprPerformance31dExecution.plus(aprPerformance31dConsensus)
			),
		...
		if (activeValidators.length > 0) {
			attrEffectiveness = sumBigInt(validators, (cur) =>
				cur.attrEffectiveness
					? new BigNumber(cur.attrEffectiveness.toString())
					: new BigNumber(0)
			)
		...
		const rocketpoolValiCount = sumBigInt(validators, (cur) =>
			cur.rocketpool ? new BigNumber(1) : new BigNumber(0)
		)
		const feeSum = sumBigInt(validators, (cur) =>
			cur.rocketpool
				? new BigNumber(cur.rocketpool.minipool_node_fee).multipliedBy('100')
				: new BigNumber('0')
		)
		var feeAvg = 0
		...
		return {
			...
			lazyLoadChart: true,
			lazyChartValidators: getValidatorQueryString(
				validators,
				2000,
				this.userMaxValidators - 1
			),
			...
			currentSyncCommittee: currentSync
				? currentSync.currentSyncCommittee
				: null,
			...
			rocketpool: {
				...
				fee: feeAvg,
				status:
					foreignValidator && validators[0].rocketpool
						? validators[0].rocketpool.minipool_status
						: null,
				depositType:
					foreignValidator && validators[0].rocketpool
						? validators[0].rocketpool.minipool_deposit_type
						: null,
				...
				smoothingPool:
					validators.find((cur) =>
						cur.rocketpool
							? cur.rocketpool.smoothing_pool_opted_in == true
							: false
					) != null
						? true
						: false,
				hasNonSmoothingPoolAsWell:
					validators.find((cur) =>
						cur.rocketpool
							? cur.rocketpool.smoothing_pool_opted_in == false
							: true
					) != null
						? true
						: false,
	...
	private getExecutionPerformance(
		...
		const performance1d = this.sumBigIntPerformanceRpl(validators, (cur) =>
			this.sumExcludeSmoothingPool(cur, (cur) =>
				cur.execution.performance1d.toString()
			).multipliedBy(new BigNumber(cur.execshare == null ? 1 : cur.execshare))
		)
		...
		const performance7d = this.sumBigIntPerformanceRpl(validators, (cur) =>
			this.sumExcludeSmoothingPool(cur, (cur) =>
				cur.execution.performance7d.toString()
			).multipliedBy(new BigNumber(cur.execshare == null ? 1 : cur.execshare))
		)

		const aprExecution = this.getAPRFromMonth(
			effectiveBalanceActive,
			aprPerformance31dExecution
		) // todo
		return {
	...
	sumExcludeSmoothingPool(cur: Validator, field: (cur: Validator) => string) {
		...
		if (!cur.rocketpool || !cur.rocketpool.smoothing_pool_opted_in) {
			return cur.execution
				? new BigNumber(field(cur)).dividedBy(new BigNumber('1e9'))
				: new BigNumber(0)
		}
	...
	private getConsensusPerformance(
		...
		const performance1d = this.sumBigIntPerformanceRpl(validators, (cur) =>
			new BigNumber(cur.data.performance1d).multipliedBy(
				new BigNumber(cur.share == null ? 1 : cur.share)
			)
		)
		const performance31d = this.sumBigIntPerformanceRpl(validators, (cur) =>
			new BigNumber(cur.data.performance31d).multipliedBy(
				new BigNumber(cur.share == null ? 1 : cur.share)
			)
		)
		const performance7d = this.sumBigIntPerformanceRpl(validators, (cur) =>
			new BigNumber(cur.data.performance7d).multipliedBy(
				new BigNumber(cur.share == null ? 1 : cur.share)
			)
		)
		const performance365d = this.sumBigIntPerformanceRpl(validators, (cur) =>
			new BigNumber(cur.data.performance365d).multipliedBy(
				new BigNumber(cur.share == null ? 1 : cur.share)
			)
		)

		const aprConsensus = this.getAPRFromMonth(
			effectiveBalanceActive,
			aprPerformance31dConsensus
		)
	...
	getRocketpoolCheatingStatus(
		validators: Validator[]
	): RocketpoolCheatingStatus {
		var strikes = 0
		var penalties = 0
	...
	sumBigIntBalanceRpl<T>(
		validators: Validator[],
		field: (cur: Validator) => BigNumber
	): BigNumber {
		return sumBigInt(validators, (cur) => {
			const fieldVal = field(cur)
			if (!cur.rocketpool)
				return fieldVal.multipliedBy(
					new BigNumber(cur.share == null ? 1 : cur.share)
				)
			...
			const rewards = new BigNumber(fieldVal.toString()).minus(
				cur.data.effectivebalance
			)
			const nodeOperatorRewards = new BigNumber(rewards)
				.multipliedBy(
					new BigNumber('1').plus(
						new BigNumber(cur.rocketpool.minipool_node_fee.toString())
					)
				)
				.dividedBy('2')
			const wholeBalance = new BigNumber('16000000000').plus(
				nodeOperatorRewards
			)
			return wholeBalance.multipliedBy(
				new BigNumber(cur.share == null ? 1 : cur.share)
			)
		})
	}

	sumBigIntPerformanceRpl<T>(
		validators: Validator[],
		field: (cur: Validator) => BigNumber
	): BigNumber {
		return sumBigInt(validators, (cur) => {
	...
	getDashboardState(
		...
		if (
			activeValidatorCount == requiredValidatorsForOKState &&
		...
		const awaitingActivation =
			this.getWaitingForActivationValidators(validators)
	...
	private getOkState(
		...
		const exitingDescription = this.getExitingDescription(
			validatorResp,
			currentEpoch
		)
	...
	private getExitingDescription(
		validatorResp: ValidatorResponse,
		currentEpoch: EpochResponse
	): Description {
		if (!validatorResp.exitepoch)
			return { extendedDescriptionPre: null, extendedDescription: null }

		const exitDiff = validatorResp.exitepoch - currentEpoch.epoch
		const isExiting = exitDiff >= 0 && exitDiff < 6480 // ~ 1 month
		const exitingDate = isExiting
			? this.getEpochDate(validatorResp.exitepoch, currentEpoch)
			: null
	...
	private getAwaitingActivationState(
		...
		const pre = allAffected ? 'All' : 'Some'
		const estEta = this.getEpochDate(
			awaitingActivation.activationeligibilityepoch,
			currentEpoch
		)
		return {
			icon: 'timer-outline',
	...
	private getEligbableActivationState(
		...
		const pre = allAffected ? 'All' : 'Some'
		const estEta = this.getEpochDate(
			eligbleState.activationeligibilityepoch,
			currentEpoch
		)
		return {
			icon: 'timer-outline',
	...
	private getExitedState(
		activeValidatorCount,
		validatorCount,
		foreignValidator
	): DashboardStatus {
		return {
			icon: this.descriptionSwitch(
				'ribbon-outline',
				'home-outline',
				foreignValidator
			),
			...
			extendedDescription: this.descriptionSwitch(
