...
export class Tab1Page {
	...
	private async removeTooltips() {
		var inputs = Array.from(
			document.getElementsByTagName('tooltip') as HTMLCollectionOf<HTMLElement>
		)
	...
	async refresh() {
		...
		const validators = await this.validatorUtils
			.getAllMyValidators()
			.catch((error) => {
				return []
			})
		const epoch = await this.validatorUtils
			.getRemoteCurrentEpoch()
			.catch((error) => {
				return null
			})
