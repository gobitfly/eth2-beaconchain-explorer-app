...
export class StorageService extends CacheModule {
	...
	async loadPreferencesToggles(network: string): Promise<boolean> {
		const notifyLocal = await this.getBooleanSetting(
			network + SETTING_NOTIFY,
			null
		)
		return notifyLocal
	}
	...
	async migrateToCapacitor3() {
		...
		let alreadyMigrated = await this.getBooleanSetting(
			'migrated_to_cap3',
			false
		)
		if (!alreadyMigrated) {
	...
	async openLogSession(modalCtr, offset: number) {
		var lastLogSession = parseInt(
			await window.localStorage.getItem('last_log_session')
		)
			...
			componentProps: {
				logs: JSON.parse(
					window.localStorage.getItem(
						'log_session_' + ((lastLogSession + (3 - offset)) % 3)
					)
				),
			},
		})
	...
	async isNotifyClientUpdatesEnabled(): Promise<boolean> {
		return (
			(await this.getBooleanSetting('eth_client_update', true)) &&
