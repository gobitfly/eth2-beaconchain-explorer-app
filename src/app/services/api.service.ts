...
export class ApiService extends CacheModule {
	...
	private async getAuthHeader(isTokenRefreshCall: boolean) {
		...
		if (
			!isTokenRefreshCall &&
			user.expiresIn <= Date.now() - (SERVER_TIMEOUT + 1000)
		) {
			...
			if (!user || !user.accessToken) {
				...
				if (
					!isNaN(markForLogoutInt) &&
					markForLogoutInt + 12 * 60 * 1000 < Date.now()
				) {
	...
	private async getCacheKey(request: APIRequest<any>): Promise<string> {
		if (request.method == Method.GET) {
			return (
				request.method +
				(await this.getResourceUrl(request.resource, request.endPoint))
			)
	...
	async execute(request: APIRequest<any>) {
		...
		if (
			(request.method == Method.GET || request.cacheablePOST) &&
			result &&
			result.status == 200 &&
			result.data
		) {
			this.putCache(
				await this.getCacheKey(request),
				result,
				request.maxCacheAge
			)
		}
	...
	private async get(
		...
		return Http.get(getOptions)
			...
			.then((response: Response) =>
				this.validateResponse(ignoreFails, response)
			)
	}
	...
	private async legacyGet(
		...
		return this.httpLegacy
			...
			.then((response: AxiosResponse<any>) =>
				this.validateResponseLegacy(ignoreFails, response)
			)
	}
    ...
    */
	...
	private getContentType(data: any): string {
		if (data instanceof FormDataContainer)
	...
	private validateResponseLegacy(
		ignoreFails,
