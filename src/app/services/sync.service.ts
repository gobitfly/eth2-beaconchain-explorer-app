...
export class SyncService {
	...
	async syncAllSettings(useForce: boolean = false) {
		...
		if (!useForce && this.lastNotifySync + 15 * 1000 > Date.now()) {
			console.log(
				'Syncing notify cancelled, last update was less than 15 seconds ago'
			)
			return
		}
		...
		console.log(
			'== Step 1: Loading notification preferences from beaconcha.in =='
		)
	...
	async syncAllSettingsForceStaleNotifications() {
		...
		for (const key of allNotifyKeys) {
			...
			if (syncAction == SyncActionEvent.NOTIFICATIONS) {
				...
				await this.setLastChanged(
					cleanKey,
					temp.eventName,
					temp.eventFilter,
					temp.eventThreshold,
					temp.subscribeAction
				)
			}
		}
	...
	private getSyncAction(
		...
		if (actionEvent == SyncActionEvent.NOTIFY_GLOBAL) {
			return async (syncChange: SyncChanged, onComplete) => {
				const result = await this.updateRemoteGeneralNotifySettings(
					syncChange.subscribeAction != 'unsubscribe'
				)
				onComplete(result)
				return result
			}
	...
	private getClientUpdateSyncAction() {
		return async (
			...
			return this.getNotifySubSyncAction(syncChange.eventName)(
				syncChange,
				superOnComplete
			)
		}
	...
	async postNotifyBundles() {
		...
		for (var i = 0; i < bundles.length; i++) {
			...
			while (bundle.bundles.length) {
				...
				let success = await this.postNotifyBundle(
					bundle.subscribe,
					splice,
					bundle.network
				)
	...
	private async postNotifyBundle(
		subscribe: boolean,
		data: BundleSub[],
		network: string
	): Promise<boolean> {
	...
	private async syncSingle(
		...
		if (syncChange.lastChanged > syncChange.lastSynced) {
			console.log(
				'== Syncing notify: ' + key + ' changed, updating on remote... ==',
				syncChange.lastChanged,
				syncChange.lastSynced
			)
	...
	async changeClient(clientKey: string, value: string, force: boolean = false) {
		...
		if (value && value != 'NULL') {
			...
			this.setLastChanged(
				'UN' + client.storageKey,
				'eth_client_update',
				client.key.toLocaleLowerCase(),
				null,
				'unsubscribe'
			)
		}
	...
	async reapplyNotifyEvent(
		...
		if (currentKey == null) {
			console.log(
				'ignoring reapplyNotifyEvent, no key with similar name found',
				event,
				event
			)
			return false
		...
		if (current.eventName == '') {
			console.log(
				'ignoring reapplyNotifyEvent since no previous setting change has been found for event',
				event,
				event
			)
			return false
		...
		if (!subscribeAction) {
			subscribeAction = (await this.storage.getBooleanSetting(
				current.eventName
			))
				? 'subscribe'
				: 'unsubscribe'
		...
		this.setLastChanged(
			...
			event,
			filter,
			current.eventThreshold,
			subscribeAction
		)
		return true
	}

	async changeNotifyClientUpdate(
		key: string,
		value: boolean,
		filter: string = null
	) {
	...
	private async getChanged(key: string): Promise<SyncChanged> {
		const temp: SyncChanged = await this.storage.getObject(
			NOTIFY_SYNCCHANGE + key
		)
