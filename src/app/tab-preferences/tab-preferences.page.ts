...
export class Tab3Page {
	...
	ngOnInit() {
		...
		this.updateUtils.getClient('ROCKETPOOL').then((result) => {
			this.notificationBase.setClientToggleState(
				'ROCKETPOOL',
				result && result.toUpperCase() == 'ROCKETPOOL'
			)
		})
		this.updateUtils.getClient('MEV-BOOST').then((result) => {
			this.notificationBase.setClientToggleState(
				'MEV-BOOST',
				result && result.toUpperCase() == 'MEV-BOOST'
			)
		})
		this.updateUtils
			.getUpdateChannel()
			.then((result) => (this.updateChannel = result))
	...
	rateApp() {
		if (this.platform.is('android')) {
			window.open(
				'market://details?id=in.beaconcha.mobile',
				'_system',
				'location=yes'
			)
		} else {
			window.open(
				'itms-apps://itunes.apple.com/app/id1541822121',
				'_system',
				'location=yes'
			)
		}
