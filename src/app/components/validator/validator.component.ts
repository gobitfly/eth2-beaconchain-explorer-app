...
export class ValidatorComponent implements OnInit {
  ...
  ngOnInit() {
    setTimeout(() => { this.fadeIn = null }, 500)
  }
  ...
  untag(event) {
    ...
    this.alerts.confirmDialog(
      'Remove validator',
      'Do you want to remove this validator?',
      'Delete',
      () => { this.confirmUntag() }
    );
  }
  ...
  interpretState(item: Validator) {
    switch (item.state) {
      case ValidatorState.ACTIVE: return "Active"
      case ValidatorState.OFFLINE: return "Offline"
      case ValidatorState.SLASHED: return "Slashed"
      case ValidatorState.EXITED: return "Exited"
      case ValidatorState.WAITING: return "Waiting for Activation"
      case ValidatorState.ELIGABLE: return "Waiting for deposit processing"
  ...
  interpretStateCss(item: Validator) {
    switch (item.state) {
      case ValidatorState.ACTIVE: return "online"
      case ValidatorState.OFFLINE: return "offline"
      case ValidatorState.SLASHED: return "slashed"
      case ValidatorState.EXITED: return "exited"
      case ValidatorState.WAITING: return "waiting"
      case ValidatorState.ELIGABLE: return "waiting"
      default: return ""
    }
  }
