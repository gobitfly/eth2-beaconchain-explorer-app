...
export class DashboardComponent implements OnInit {
	...
	updateMergeListDismissed() {
		this.storage
			.getBooleanSetting('merge_list_dismissed', false)
			.then((result) => {
	...
	async updateActiveSyncCommitteeMessage(committee: SyncCommitteeResponse) {
		if (committee) {
			...
			this.currentSyncCommitteeMessage = {
				...
				text: `Your validator${
					committee.validators.length > 1 ? 's' : ''
				} ${committee.validators.toString()} ${
					committee.validators.length > 1 ? 'are' : 'is'
				} currently part of the active sync committee.
      <br/><br/>This duty started at epoch ${
				committee.start_epoch
			} at ${new Date(startTs).toLocaleString()} and 
      will end at epoch ${committee.end_epoch} at ${new Date(
					endTs
				).toLocaleString()}. 
	...
	async updateNextSyncCommitteeMessage(committee: SyncCommitteeResponse) {
      ...
      <br/><br/>This duty starts at epoch ${
				committee.start_epoch
			} at ${new Date(startTs).toLocaleString()} and 
      will end at epoch ${committee.end_epoch} at ${new Date(
					endTs
	...
	updateSmoothingPool() {
		try {
			this.hasNonSmoothingPoolAsWell =
				this.data.rocketpool.hasNonSmoothingPoolAsWell
			this.displaySmoothingPool = this.data.rocketpool.smoothingPool
			;(this.smoothingClaimed =
				this.data.rocketpool.smoothingPoolClaimed.dividedBy(
					new BigNumber('1e9')
				)),
				(this.smoothingUnclaimed =
					this.data.rocketpool.smoothingPoolUnclaimed.dividedBy(
						new BigNumber('1e9')
					)),
	...
	updateRplProjectedClaim() {
      ...
      if (totalRplAtNextCheckpoint < 0) {
          totalRplAtNextCheckpoint = 0
			...
			if (
				this.data.rocketpool.currentRpl.isLessThanOrEqualTo(
					this.data.rocketpool.minRpl
				)
			) {
			...
			const temp = this.getEffectiveRplStake(this.data.rocketpool)
				.dividedBy(
					new BigNumber(
						this.validatorUtils.rocketpoolStats.effective_rpl_staked
					)
				)
				//.multipliedBy(new BigNumber(totalRplAtNextCheckpoint.toString()))
				.multipliedBy(
					new BigNumber(
						this.validatorUtils.rocketpoolStats.node_operator_rewards
					)
				)
	...
	updateRplApr() {
		try {
			const hoursToAdd =
				this.validatorUtils.rocketpoolStats.claim_interval_time.split(':')[0]
			const hoursNumber = parseInt(hoursToAdd)
			this.rplApr = new BigNumber(
				this.validatorUtils.rocketpoolStats.node_operator_rewards
			)
				.dividedBy(
					new BigNumber(
	...
	updateRplCommission() {
		try {
			this.rplCommission =
				Math.round(
					this.validatorUtils.rocketpoolStats.current_node_fee * 10000
				) / 100
		} catch (e) {}
	}
	...
	updateNextRewardRound() {
		try {
			const hoursToAdd =
				this.validatorUtils.rocketpoolStats.claim_interval_time.split(':')[0]
			this.nextRewardRound =
				this.validatorUtils.rocketpoolStats.claim_interval_time_start * 1000 +
	...
	ngOnInit() {
		...
		this.storage
			.getBooleanSetting('rank_percent_mode', false)
			.then((result) => (this.rankPercentMode = result))
		this.storage
			.getItem('rpl_pdisplay_mode')
			.then((result) => (this.rplState = result ? result : 'rpl'))
	...
	async checkForGenesisOccured() {
		...
		this.awaitGenesis =
			currentEpoch.epoch == 0 && currentEpoch.proposedblocks <= 1
		this.earlyGenesis =
			!this.awaitGenesis && !this.finalizationIssue && currentEpoch.epoch <= 7
	}

	async checkForFinalization() {
		const cachedFinalizationIssue = await this.storage.getObject(
			'finalization_issues'
		)
		if (cachedFinalizationIssue) {
			if (
				cachedFinalizationIssue.ts &&
				cachedFinalizationIssue.ts + 4 * 60 * 60 * 1000 > Date.now()
			) {
				console.log(
					'returning cached finalization issue state',
					cachedFinalizationIssue
				)
		...
		this.finalizationIssue =
			new BigNumber(olderResult.globalparticipationrate).isLessThan('0.664') &&
			olderResult.epoch > 7
		this.storage.setObject('finalization_issues', {
			ts: Date.now(),
			value: this.finalizationIssue,
		})
	}

	async getChartData(data: 'allbalances' | 'proposals') {
		if (!this.data || !this.data.lazyChartValidators) return null
		const chartReq = new DasboardDataRequest(
			data,
			this.data.lazyChartValidators
		)
	...
	updateRplDisplay() {
		if (this.rplState == '%') {
			this.rplDisplay = this.data.rocketpool.currentRpl
				.dividedBy(this.data.rocketpool.maxRpl)
				.multipliedBy(new BigNumber(150))
				.decimalPlaces(1)
		} else {
			this.rplDisplay = this.data.rocketpool.currentRpl
		}
	...
	private async checkForFirstProposal(chartData) {
		...
		const noPreviousFirstProposal = await this.storage.getBooleanSetting(
			'first_proposal_executed',
			false
		)
	...
	createProposedChart(proposed, missed, orphaned) {
		const chart = Highstock.stockChart(
			// @ts-ignore     ¯\_(ツ)_/¯
		...
		Highstock.chart('highcharts' + this.randomChartId, {
			...
			tooltip: {
				...
				formatter: (tooltip) => {
					...
					).toLocaleDateString()} ${getEpochString(
						tooltip.chart.hoverPoints[0].x
					)}`
			...
			yAxis: [
				{
					...
					tickPositioner: function () {
						...
						const min =
							Math.round(this.chart.series[1].dataMin * padding * precision) /
							precision
						const max =
							Math.round(this.chart.series[1].dataMax * padding * precision) /
							precision
					...
					labels: {
						...
						formatter: function () {
							...
							return parseFloat(this.value.toString()).toFixed(
								ticksDecimalPlaces
							)
						},
					},
	...
	async getBrowserURL(): Promise<string> {
		if (this.data.foreignValidator) {
			return (
				(await this.getBaseBrowserUrl()) +
				'/validator/' +
				this.data.foreignValidatorItem.pubkey
			)
		} else {
			return (
				(await this.getBaseBrowserUrl()) +
