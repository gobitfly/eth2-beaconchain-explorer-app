...
export class ValidatorUtils extends CacheModule {
	...
	private async getMapWithoutDeleted(
		storageKey
	): Promise<Map<String, Validator>> {
	...
	async saveRocketpoolCollateralShare(
		nodeAddress: string,
		sharePercent: number
	) {
	...
	async getLocalValidatorIndexes(): Promise<string> {
		...
		return getValidatorQueryString(
			[...local.values()],
			2000,
			(await this.merchantUtils.getCurrentPlanMaxValidator()) - 1
		)
	}
	...
	async getAllMyValidators(): Promise<Validator[]> {
		...
		const validatorString = getValidatorQueryString(
			[...local.values()],
			2000,
			(await this.merchantUtils.getCurrentPlanMaxValidator()) - 1
		)
		...
		const remoteUpdatesPromise = this.getDashboardDataValidators(
			SAVED,
			validatorString
		).catch((err) => {
			return []
	...
	public async getAllNewIndizesOnly(
		myRemotes: MyValidatorResponse[]
	): Promise<number[]> {
		const storageKey = await this.getStorageKey()
		const current = await this.getMap(storageKey)
	...
	getValidatorState(
		...
		if (item.data.activationeligibilityepoch > currentEpoch.epoch)
			return ValidatorState.ELIGABLE
		if (item.data.activationepoch > currentEpoch.epoch)
			return ValidatorState.WAITING
		if (item.data.lastattestationslot < (currentEpoch.epoch - 2) * 32)
	...
	async getDashboardDataValidators(
		storage: 0 | 1,
		...validators
	): Promise<Validator[]> {
...
*/
		...
		if (!request.wasSuccessfull(response)) {
			if (
				...
				response.data.status.indexOf('only a maximum of') >= 0
			) {
		...
		const temp = this.convertToValidatorModel({
			synced: false,
			storage: storage,
		...
		for (let vali of temp) {
			vali.attrEffectiveness = this.findAttributionEffectiveness(
				validatorEffectivenessResponse,
				vali.index
			)
			vali.rocketpool = this.findRocketpoolResponse(
				result.rocketpool_validators,
				vali.index
			)
			vali.execution = this.findExecutionResponse(
				result.execution_performance,
				vali.index
			)
			if (local) {
				...
				if (found) {
					...
					vali.execshare =
						found.execshare == null || found.execshare == undefined
							? found.share
							: found.execshare
				}
			}
			if (vali.rocketpool) {
				vali.rplshare = await this.getRocketpoolCollateralShare(
					vali.rocketpool.node_address
				)
			}

			if (
				result.current_sync_committee &&
				result.current_sync_committee.length >= 1
			) {
				vali.currentSyncCommittee = this.findSyncCommitteeDuty(
					result.current_sync_committee[0],
					vali.index
				)
			}
			if (
				result.next_sync_committee &&
				result.next_sync_committee.length >= 1
			) {
				vali.nextSyncCommittee = this.findSyncCommitteeDuty(
					result.next_sync_committee[0],
					vali.index
				)
			}
		}
	...
	private findSyncCommitteeDuty(
		committee: SyncCommitteeResponse,
		index: number
	): SyncCommitteeResponse {
	...
	private updateRplAndRethPrice() {
		...
		this.unitConversion.setRPLPrice(
			new BigNumber(this.rocketpoolStats.rpl_price.toString())
		)
		this.unitConversion.setRETHPrice(
			new BigNumber(this.rocketpoolStats.reth_exchange_rate.toString())
		)
	}

	private findExecutionResponse(
	...
	private findRocketpoolResponse(
		list: RocketPoolResponse[],
		index: number
	): RocketPoolResponse {
		for (let attr of list) {
			if (attr.index == index) {
	...
	private findAttributionEffectiveness(
		list: AttestationPerformanceResponse[],
		index: number
	): number {
		for (let attr of list) {
			if (attr.validatorindex == index && attr.attestation_efficiency) {
				return new BigNumber(1)
					.dividedBy(attr.attestation_efficiency)
					.multipliedBy(100)
					.decimalPlaces(1)
					.toNumber()
			}
	...
	async getRemoteValidatorViaETH1(
		arg: string,
		enforceMax: number = -1
	): Promise<Validator[]> {
		if (!arg) return []
		...
		if (request.wasSuccessfull(response)) {
			...
			const queryString = getValidatorQueryString(
				eth1ValidatorList,
				2000,
				enforceMax
			)
	...
	// single
	async convertToValidatorModelAndSaveValidatorLocal(
		synced: boolean,
	...
	// multiple
	async convertToValidatorModelsAndSaveLocal(
		synced: boolean,
		validator: ValidatorResponse[]
	) {
		this.saveValidatorsLocal(
			this.convertToValidatorModel({
				synced,
				storage: SAVED,
	...
	async searchValidators(search: string): Promise<Validator[]> {
		const result = await this.getDashboardDataValidators(MEMORY, search).catch(
			(err) => {
				return null
			}
		)
...
export function getValidatorQueryString(
	validators: any[],
	getParamMaxLimit: number,
	maxValLimit: number = -1
) {
	// Validator
	var erg = ''
	var count = 0
