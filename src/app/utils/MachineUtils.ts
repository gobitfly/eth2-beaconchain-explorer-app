...
export default class MachineUtils extends CacheModule {
    ...
    */
	...
	private async registerNewRemotesForSync(remote: string[]): Promise<boolean> {
		...
		for (var i = 0; i < remote.length; i++) {
			...
			if (!local || !local.includes(it)) {
				console.log(
					LOGTAG +
						'found a new machine, applying your notification preferences...'
				)
				returnOK =
					(await this.sync.reapplyNotifyEvent(
						'monitoring_machine_offline',
						it
					)) && returnOK
				returnOK =
					(await this.sync.reapplyNotifyEvent(
						'monitoring_hdd_almostfull',
						it
					)) && returnOK
				returnOK =
					(await this.sync.reapplyNotifyEvent('monitoring_cpu_load', it)) &&
					returnOK
				returnOK =
					(await this.sync.reapplyNotifyEvent('monitoring_memory_usage', it)) &&
					returnOK
			}
		}
	...
	private unsupportedPrysmVersion(
		data: ProcessedStats[],
	...
	private async getAndProcessDataBase(timeslot: number = 180) {
		...
		const unsupportedPrysm = this.unsupportedPrysmVersion(
			result,
			machineController
		)
