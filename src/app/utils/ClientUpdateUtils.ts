...
export default class ClientUpdateUtils {
	...
	async dismissRelease(clientKey: string, id: string) {
		this.storage.setObject(LOCAL_UPDATED_KEY + clientKey, {
			clientKey: clientKey,
			version: id,
		})
	}

	private async getLastClosedVersion(
		clientKey: string
	): Promise<LocalReleaseMark> {
	...
	private async getReleases(client: ClientInfo): Promise<Release> {
		const req = new GithubReleaseRequest(
			client.repo,
			!(await this.isPreReleaseAllowed())
		)
