...
export class MerchantUtils {
	...
	private async registerPurchaseOnRemote(
	...
	private async confirmPurchaseOnRemote(product) {
		if (product.id == 'in.beaconcha.mobile') {
			this.alertService.showError(
				'Purchase Error',
				'Invalid product, try again later or report this issue to us if persistent.',
				PURCHASEUTILS + 4
			)
			return
		}
		...
		const loading = await this.alertService.presentLoading(
			'Confirming, this might take a couple seconds'
		)
		loading.present()
		...
		if (result) {
			this.alertService.confirmDialog(
				'Upgrade successfull',
				'App requires a restart. Do you want to restart it now?',
				'Restart App',
				() => {
	...
	async getCurrentPlanConfirmed(): Promise<string> {
		if (this.api.debug) {
			const debugPackage = await this.storage.getSetting(
				DEBUG_SETTING_OVERRIDE_PACKAGE,
				'default'
			)
	...
	async hasPremiumTheming() {
		const currentPlan = await this.getCurrentPlanConfirmed()
		return (
			currentPlan != PRODUCT_STANDARD &&
			currentPlan != '' &&
			currentPlan != 'plankton'
		)
	}
