...
export default class FirebaseUtils {
	...
	async hasSeenConsentScreenAndNotConsented() {
		const hasSeenConsent = await this.storage.getBooleanSetting(
	...
	async registerPush(
		iosTriggerConsent = false,
		onPermissionGranted: () => any = null
	) {
		if (!this.platform.is('ios') && !this.platform.is('android')) {
			return
		}
	...
	async pushLastTokenUpstream(force: boolean) {
		this.updateRemoteNotificationToken(
			await this.storage.getItem(CURRENT_TOKENKEY),
			force
		)
	}
	...
	private async updateRemoteNotificationToken(token: string, force = false) {
		console.log(
			LOGTAG + ' updateRemoteNotificationToken called with token: ' + token
		)
