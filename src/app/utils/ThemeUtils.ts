...
export default class ThemeUtils {
	...
	async init(splashScreenCallback: () => void) {
		this.lock = this.storage
			.getObject(STORAGE_KEY)
			.then((preferenceDarkMode) => {
	...
	private internalInit(preferenceDarkMode) {
		...
		this.toggle(
			this.userPreference == Theme.DARK,
			false,
	...
	async toggle(
		...
		setColorHandler: boolean = true,
		themeColor: string = this.currentThemeColor
	) {
		document.body.classList.toggle('dark', darkModeEnabled)
		if (themeColor && themeColor != '')
			document.body.classList.toggle(themeColor, true)
	...
	isSilvester() {
		var d = new Date()
		const silvesterday =
			d.getMonth() == 11 && d.getDate() == 31 && d.getHours() == 23
		const januaryFirst =
			d.getMonth() == 0 && d.getDate() == 1 && d.getHours() == 0
	...
	toggleWinter(enabled, saveWinterSetting = true) {
		...
		if (this.isWinterSeason() && saveWinterSetting)
			this.storage.setBooleanSetting('snow_enabled', enabled)
