...
export class ValuestylePipe implements PipeTransform {
	transform(tempvalue: any, ...args: any[]): string {
		const value =
			tempvalue instanceof BigNumber ? tempvalue : new BigNumber(tempvalue)
		const firstDrop =
			args[0] instanceof BigNumber ? args[0] : new BigNumber(args[0])

		if (args.length == 2) {
			const secondDrop =
				args[1] instanceof BigNumber ? args[1] : new BigNumber(args[1])
