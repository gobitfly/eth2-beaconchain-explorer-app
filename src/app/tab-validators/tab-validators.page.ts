...
export class Tab2Page {
	...
	private async refreshMyValidators() {
		...
		temp = temp.sort((a, b) => {
			if (
				a.state == ValidatorState.SLASHED &&
				b.state != ValidatorState.SLASHED
			) {
				return -1
			}
			if (
			...
			if (
				a.state == ValidatorState.OFFLINE &&
				b.state == ValidatorState.OFFLINE
			) {
				return -1
			}
			if (
			...
			if (
				a.state == ValidatorState.WAITING &&
				b.state == ValidatorState.WAITING
			) {
				return -1
			}
			if (
	...
	async removeAllDialog() {
		this.showDialog(
			'Remove all',
			'Do you want to remove {AMOUNT} validators from your dashboard?',
			() => {
	...
	async addAllDialog() {
		this.showDialog(
			'Add all',
			'Do you want to add {AMOUNT} validators to your dashboard?',
			() => {
				this.confirmAddAll()
			}
		)
	...
	async searchFor(searchString) {
		...
		const isETH1Address =
			searchString.startsWith('0x') && searchString.length == 42

		if (isETH1Address)
			this.searchETH1(searchString).then(() => (this.loading = false))
		else
			this.searchByPubKeyOrIndex(searchString).then(
				() => (this.loading = false)
			)
	}

	async searchByPubKeyOrIndex(target) {
		const temp = await this.validatorUtils
		...
		const temp = await this.validatorUtils
			...
			.catch(async (error) => {
				if (
					error &&
					error.message &&
					error.message.indexOf('only a maximum of') > 0
				) {
					console.log('SET reachedMaxValidators to true')
					this.reachedMaxValidators = true
					return await this.validatorUtils.searchValidatorsViaETH1(
						target,
						this.currentPackageMaxValidators - 1
					)
				}
				return []
			})
	...
	async setRPLShares() {
		...
		const validatorSubArray = this.getValidatorsByIndex(this.selected).filter(
			(item) => {
		...
		const onlyOneNodeAddress = validatorSubArray.reduce((prev, cur) => {
			return prev && prev.rocketpool.node_address == cur.rocketpool.node_address
				? cur
				: null
		})
		if (!onlyOneNodeAddress) {
			console.log(
				'You selected validators belonging to multiple node addresses. Currently only one at a time can be edited.'
			)
			Toast.show({
		...
		const maxStakeShare = new BigNumber(
			onlyOneNodeAddress.rocketpool.node_rpl_stake
		)
			.dividedBy(new BigNumber(1e18))
			.decimalPlaces(2)

		var currentShares = await this.validatorUtils.getRocketpoolCollateralShare(
			onlyOneNodeAddress.rocketpool.node_address
		)
		var current = null
		if (currentShares) {
			current = new BigNumber(currentShares)
				.multipliedBy(new BigNumber(maxStakeShare))
				.decimalPlaces(4)
		}
			...
			buttons: [
				{
					text: 'Remove',
					handler: async (_) => {
						this.validatorUtils.saveRocketpoolCollateralShare(
							onlyOneNodeAddress.rocketpool.node_address,
							null
						)
						this.cancelSelect()
				...
				},
				{
					...
					handler: async (alertData) => {
						...
						if (shares < minShareStake) {
							Toast.show({
								text:
									'Share must be at least ' + minShareStake + ' RPL or more.',
							})
							return
						}
						...
						const share = new BigNumber(alertData.share).div(
							new BigNumber(maxStakeShare)
						)
						this.validatorUtils.saveRocketpoolCollateralShare(
							onlyOneNodeAddress.rocketpool.node_address,
							share.toNumber()
	...
	async setShares() {
		...
		const current = new BigNumber(this.getCurrentShare(validatorSubArray))
			.multipliedBy(new BigNumber(maxStakeShare))
			.decimalPlaces(4)

		const currentEL = new BigNumber(this.getCurrentELShare(validatorSubArray))
			.multipliedBy(new BigNumber(maxStakeShare))
			.decimalPlaces(4)
			...
			buttons: [
				...
				},
				{
					...
					handler: async (alertData) => {
						...
						if (
							(shares && shares < minShareStake) ||
							(sharesEL && sharesEL < minShareStake)
						) {
							Toast.show({
								text:
									'Share must be at least ' + minShareStake + ' ETH or more',
							})
							return
						}

						if (
							(shares && shares > maxStakeShare) ||
							(sharesEL && shares > maxStakeShare)
						) {
							Toast.show({
						...
						const shareEL = new BigNumber(alertData.execshare).div(
							new BigNumber(maxStakeShare)
						)
