...
export abstract class APIRequest<T> {
	...
	wasSuccessfull(response: any, hasDataStatus = true): boolean {
		if (this.nativeHttp) {
			...
			return (
				response.status == 200 &&
				(response.data.status == 'OK' || !hasDataStatus)
			)
		} else {
			return (
				response &&
				(response.status == 'OK' || response.status == 200 || !hasDataStatus)
			)
		}
	}
...
export class AttestationPerformanceRequest extends APIRequest<AttestationPerformanceResponse> {
	...
	constructor(...validator: any) {
		super()
		this.resource +=
			validator.join().replace(/\s/g, '') + '/attestationefficiency'
	}
}
...
export class NotificationSubsRequest extends APIRequest<ApiTokenResponse> {
	...
	constructor(eventName: string, filter: string = null, enabled: boolean) {
		...
		if (filter != null) {
			this.resource +=
				(enabled ? this.subscribe : this.unsubscribe) +
				'?event=' +
				eventName +
				'&filter=' +
				filter
			...
			this.resource +=
				(enabled ? this.subscribe : this.unsubscribe) + '?event=' + eventName
		}
	}
}
...
export class CoinzillaAdRequest extends APIRequest<CoinzillaAdResponse> {
	...
	parse(response: any): CoinzillaAdResponse[] {
		if (
			!this.wasSuccessfull(response, false) ||
			!response ||
			!response.data ||
			!response.data.ad
		) {
